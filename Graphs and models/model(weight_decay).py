...
def model():
    ...
    with tf.variable_scope('fully_connected') as scope:
        ...
        fc = tf.layers.dense(inputs=flat, units=1500, activation=tf.nn.relu, kernel_regularizer = tf.contrib.layers.l2_regularizer(0.001))
        
        softmax = tf.layers.dense(inputs=fc, units=_NUM_CLASSES, name=scope.name)
